    ...
    const form = {
      ...
      fields: [
        ...
        },
        {
          ...
          tooltip:
            "Provide a single CSV file path or use '*' for matching multiple files. Extensions accepted: .csv, .tsv, .txt. Can also read CSV files compressed as .gz, .bz2, .zip, .xz, .zst.",
        ...
        },
        {
          ...
          tooltip:
            "Defines the character used to wrap fields containing special characters like the delimiter or newline.",
          advanced: true,
        },
        {
          type: "select",
        ...
        },
        {
          ...
          tooltip:
            "If you're joining by multiple columns, make sure the column lists are ordered to match the corresponding columns in the right dataset.",
          inputNb: 1,
          advanced: true,
        },
        {
...
`,
        ...
        },
        {
          type: "column",
          label:
            "25. Select a single column (column) restricted to specific types (string, bool)",
        ...
        },
        {
          type: "column",
          label:
            "27. Select a single column with default value so that code can work when retrieving default_value_column.value even if not defined(column)",
        ...
        },
        {
          ...
          aiInstructions:
            "Generate a Pandas script that return a DataFrame named 'output'. IMPORTANT: Ensure the code does not print or display anything. Include short comments for clarity.",
          aiGeneration: true,
          aiDataSample: false,
          aiPromptExamples: [
            {
...
def example_python_function(
  ...
  public generateComponentCode({ config, inputName, outputName }): string {
    ...
    const constTswith_default_value_inputNumber =
      config.with_default_value_inputNumber;
    const constTsfileLocation = config.fileLocation;
    const tsConsdatetype = JSON.stringify(config.datetype);
    const tsConsremoveUnwantedCharacters = JSON.stringify(
