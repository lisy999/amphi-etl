...
export class DateTimeConverter extends BaseCoreComponent {
  constructor() {
    ...
    const form = {
      ...
      fields: [
        ...
        },
        {
          ...
          id: "language",
          tooltip:
            "Select language or provide custom locale for your system, POSIX-style locale identifier (Linux/Mac) or Windows locale names (Windows)",
          options: [
        ...
        },
        {
          ...
          id: "dateTimeFormat",
          tooltip:
            "Select pre-defined format or provide a custom strftime format used for the conversion (in both orders)",
        ...
        },
        {
          ...
          id: "dateTimeFormat",
          tooltip:
            "Select pre-defined format, Auto detect (selected by default) or provide a custom strftime format used for the conversion (in both orders)",
    ...
    const description =
      "使用 DateTime 类可以将日期/时间格式与字符串进行转换，同时支持自定义格式设置和语言选项。";
    super(
      //   "DateTime Converter",
      "日期时间转换器",
      "datetimeConverter",
  ...
  public generateComponentCode({ config, inputName, outputName }) {
    ...
    if (newColumn) {
      const newColumnName =
        config.newColumnName && config.newColumnName.trim()
