...
export class DataCleansing extends BaseCoreComponent {
  constructor() {
    ...
    const form = {
      ...
      fields: [
        ...
        },
        {
          ...
          id: "columns",
          tooltip:
            "Select the columns to apply the cleansing rules. If left blank, all columns will be selected by default.",
        ...
        },
        {
          ...
          options: [
            ...
            },
            {
              ...
              label: "Camel Case",
              tooltip:
                "Convert to camel case, where the first letter is lowercase and subsequent words are capitalized without spaces.",
            },
            {
              value: "snakecase",
              label: "Snake Case",
              tooltip:
                "Replace spaces with underscores and convert all characters to lowercase.",
            },
          ],
          selectionRemovable: true,
        },
    ...
    const description =
      "使用数据清洗功能来清理和预处理您的数据。它提供了处理缺失值、删除空值行或列、修改字符串格式以及删除不必要字符等多种选项。您可以将这些转换应用于特定的列或整个数据集。";
    super(
      // "Data Cleansing",
      "数据清洗",
      "cleanDataCLeansing",
  ...
  public provideImports({ config }): string[] {
    ...
    if (
      config.removeUnwantedCharacters &&
  ...
  public generateComponentCode({ config, inputName, outputName }): string {
    ...
    const columnsArray =
      columns && columns.length > 0 ? columns.map(getColumnReference) : null;
    ...
    if (config.replaceMethod) {
      ...
      if (["blanks", "0", "custom"].includes(config.replaceMethod)) {
        if (columnsList) {
          code.push(
            `${outputName}[${columnsList}] = ${outputName}[${columnsList}].fillna(${value})`,
          );
        } else {
        ...
        if (columnsList) {
          code.push(
            `${outputName}[${columnsList}] = ${outputName}[${columnsList}].fillna(${outputName}[${columnsList}].median())`,
          );
        } else {
          code.push(
            `${outputName} = ${outputName}.fillna(${outputName}.median())`,
          );
        }
      } else if (["ffill", "bfill"].includes(config.replaceMethod)) {
        if (columnsList) {
          code.push(
            `${outputName}[${columnsList}] = ${outputName}[${columnsList}].fillna(method='${config.replaceMethod}')`,
          );
        } else {
          code.push(
            `${outputName} = ${outputName}.fillna(method='${config.replaceMethod}')`,
          );
        }
      }
    }
    ...
    if (Object.keys(replaceDict).length > 0 || caseTransform) {
      if (columnsList) {
        code.push(
          `${outputName}[${columnsList}] = ${outputName}[${columnsList}].astype(str)`,
        );
      } else {
      ...
      if (Object.keys(replaceDict).length > 0) {
        ...
        if (columnsList) {
          code.push(
            `${outputName}[${columnsList}] = ${outputName}[${columnsList}].replace(${replaceStr}, ${regexFlag})`,
          );
        } else {
          code.push(
            `${outputName} = ${outputName}.replace(${replaceStr}, ${regexFlag})`,
          );
        }
      }
      ...
      if (caseTransform) {
        if (columnsArray) {
          if (columnsArray.length === 1) {
            ...
            code.push(
              `${outputName}[${col}] = ${outputName}[${col}].str${caseTransform}`,
            );
          } else {
    ...
    if (config.removeNullRows) {
      if (columnsList) {
        code.push(
          `${outputName} = ${outputName}.dropna(axis=0, how='${config.removeRowsHow}', subset=${columnsList})`,
        );
      } else {
        code.push(
          `${outputName} = ${outputName}.dropna(axis=0, how='${config.removeRowsHow}')`,
        );
      }
    }
    ...
    if (config.removeNullColumns) {
      if (columnsList) {
        code.push(
          `${outputName} = ${outputName}.dropna(axis=1, how='${config.removeColumnsHow}', subset=${columnsList})`,
        );
      } else {
        code.push(
          `${outputName} = ${outputName}.dropna(axis=1, how='${config.removeColumnsHow}')`,
        );
      }
    }
