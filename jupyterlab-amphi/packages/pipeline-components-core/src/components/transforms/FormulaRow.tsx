...
export class FormulaRow extends PipelineComponent<ComponentItem>() {
  ...
  public static ConfigForm = ({
    ...
    const [formulas, setFormulas] = useState(
      data.formulas || [{ columns: [], formula: "", type: "expr" }],
    );
                    ...
                    >
                      ...
                      >
                        <SelectColumns
                          ...
                          handleChange={(value) =>
                            handleFormulaChange(value, index, "columns")
                          }
                            ...
                            options: [
                              {
                                ...
                                tooltip:
                                  "Combine variables and operators to compute a value for the selected column(s).\nExample: row['column1'].upper()",
                              },
                              {
                                value: "function",
                                label: "Python Function",
                                tooltip:
                                  "Apply a Python function to the selected column(s), with the row passed as a parameter.",
                              },
                              {
                                value: "lambda",
                                label: "Lambda Expression",
                                tooltip:
                                  "Use a Python lambda expression, an anonymous function, to process the row and apply it to the selected column(s).",
                              },
                            ],
                          }}
                          handleChange={(value) =>
                            handleFormulaChange(value, index, "type")
                          }
                      ...
                      >
                        <CodeTextarea
                          field={{
                            ...
                            aiInstructions:
                              "Use a Python expression referencing the `row` object to compute the new value. For example: row['column1'].upper(). Use the data sample to understand the schema of the incoming data.",
                            aiGeneration: false,
                            aiPromptExamples: [
                              {
                                label: "Uppercase column",
                                value: "Convert column in upper case",
                              },
                            ],
                            height: "200px",
                            mode: "python",
                          ...
                          handleChange={(value) =>
                            handleFormulaChange(value, index, "formula")
                          }
      ...
      <>
        ...
        {(showContent || isSelected) && (
          <NodeToolbar isVisible position={Position.Bottom}>
            ...
            {(FormulaRow.Type.includes("input") ||
              FormulaRow.Type.includes("processor") ||
              FormulaRow.Type.includes("output")) && (
              <button
                onClick={() => executeUntilComponent()}
                disabled={!enableExecution}
                style={{
                  opacity: enableExecution ? 1 : 0.5,
                  cursor: enableExecution ? "pointer" : "not-allowed",
                }}
              >
  ...
  public generateComponentCode({
    ...
    config.formulas.forEach((formula, index) => {
      ...
      if (formula.type === "function") {
        ...
        const extractedFunctionName = functionNameMatch
          ? functionNameMatch[1]
          : `formula_${index + 1}`;
